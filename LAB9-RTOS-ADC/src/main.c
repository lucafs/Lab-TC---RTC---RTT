...
#define NUM_TAPS_NEW 12  // ordem do filtro (quantos coefientes)
...
const float32_t firCoeffs32_OLD[NUM_TAPS] ={
	0.12269166637219883,
	0.12466396327768503,
	0.1259892807712678,
	0.12665508957884833,
...
const float32_t firCoeffs32[NUM_TAPS_NEW] ={
	0.07930125683894955,
	0.08147535648783032,
	0.08323976516671625,
	0.08457786363832452,
...
void task_lcd(void){
 ...
 void task_adc(void){
	...
	float32_t firStateF32[BLOCK_SIZE + NUM_TAPS_NEW - 1];
	float32_t inputF32[BLOCK_SIZE + NUM_TAPS_NEW - 1];
	float32_t outputF32[BLOCK_SIZE + NUM_TAPS_NEW - 1];
	arm_fir_instance_f32 S;
	arm_fir_init_f32(&S, NUM_TAPS_NEW, (float32_t *)&firCoeffs32[0], &firStateF32[0], BLOCK_SIZE);
int i = 0;
